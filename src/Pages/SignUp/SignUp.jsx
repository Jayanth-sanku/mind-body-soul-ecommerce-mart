...
//                 <input type={shouldShowPassword.pass ? "text":"password"} onKeyUp={(e)=>setPasswordInputs({...passwordInputs, pass:e.target.value})} required className="passwordInputDiv" name="password"  placeholder='Password'/>
...
//                 type={shouldShowPassword.confirmPass ? "text":"password"} onKeyUp={(e)=>setPasswordInputs({...passwordInputs, confirmPass:e.target.value})}  
//                 <input required className="confirmPasswordInputDiv" placeholder='Confirm Password'/>
...
//             disabled={passwordInputs.pass !== passwordInputs.confirmPass}
//                 <button type="submit" className='signUpActionBtn' >Sign Up</button>
//             </div>
                ...
                <div className="signUpDiv">
                    ...
                    <form onSubmit={(e) => signUpFormSubmitHandler(e)} className="signUpDetailsDiv">
                        <div className='signUpInputsDiv'>
                            ...
                            <input type={shouldShowPassword.pass ? "text" : "password"} onKeyUp={(e) => setPasswordInputs({...passwordInputs, pass:e.target.value})} required className="passwordInputDiv" name="password" placeholder='Password' />
                            {!shouldShowPassword.pass ? (
                                <svg onClick={() => showHidePassHandler(0)} className="eyeIconInvisible0" stroke="currentColor" fill="currentColor" strokeWidth="0" viewBox="0 0 1024 1024" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg">
                                    {/* Eye icon paths */}
                                </svg>
                            ) : (
                                <svg onClick={() => showHidePassHandler(0)} className="eyeIconVisible0" stroke="currentColor" fill="currentColor" strokeWidth="0" viewBox="0 0 1024 1024" width="1em" xmlns="http://www.w3.org/2000/svg">
                                    {/* Eye icon paths */}
                                </svg>
                            )}

                            <input type={shouldShowPassword.confirmPass ? "text" : "password"} onKeyUp={(e) => setPasswordInputs({...passwordInputs, confirmPass:e.target.value})} required className="confirmPasswordInputDiv" placeholder='Confirm Password' />
                            {!shouldShowPassword.confirmPass ? (
                                <svg onClick={() => showHidePassHandler(1)} className="eyeIconInvisible1" stroke="currentColor" fill="currentColor" strokeWidth="0" viewBox="0 0 1024 1024" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg">
                                    {/* Eye icon paths */}
                                </svg>
                            ) : (
                                <svg onClick={() => showHidePassHandler(1)} className="eyeIconVisible1" stroke="currentColor" fill="currentColor" strokeWidth="0" viewBox="0 0 1024 1024" width="1em" xmlns="http://www.w3.org/2000/svg">
                                    {/* Eye icon paths */}
                                </svg>
                            )}
                            {/* <p className="confirPassErrorMsg" style={{visibility:passwordInputs.pass === passwordInputs.confirmPass? "hidden":"visible"}} >
                        ...
                        <div className='newUserLogin'>
                            Already have an account? <NavLink className="newUserLoginLink" to="/login">Login</NavLink>
                        </div>
                    </form>
                </div>
            </div>
